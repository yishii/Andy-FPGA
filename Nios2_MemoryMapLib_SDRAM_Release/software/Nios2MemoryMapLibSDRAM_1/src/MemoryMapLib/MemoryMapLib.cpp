...
int MemoryMap::checkPacketHeader(MEMMAP_Message& msg) {
	...
	if ((msg.Field.Header[0] == MEMMAP_HEADER1) && (msg.Field.Header[1]
			== MEMMAP_HEADER2) && (msg.Field.Length <= MEMMAP_MAX_DATA_LENGTH)
			&& (msg.Field.Cmd >= 1) && (msg.Field.Cmd <= 4)) {
		//printf("%s Line:%d\n", __FUNCTION__, __LINE__);
		ret = true;
	}
...
int MemoryMap::processReadCommand(MEMMAP_Message& msg,
		MEMMAP_Message& msg_reply) {
	int counter;
	unsigned char address;
	unsigned char value;
	printf("%s Line:%d\n", __FUNCTION__, __LINE__);
...
int MemoryMap::registerMapAddressJob(unsigned char address, unsigned char RWOP,
		Job ptr) {
	int ret = false;
	printf("%s Line:%d\n", __FUNCTION__, __LINE__);
...
int MemoryMap::registerMapAddressVar(unsigned char address, unsigned char RWOP,
		unsigned char* ptr) {
	int ret = false;
	printf("%s Line:%d\n", __FUNCTION__, __LINE__);
...
int MemoryMap::writeMemory(unsigned char address, unsigned char value) {
	...
	if (RWOP & OPERATION_WRITE) {
		if (RWOP & OPERATION_JOB) {
			(*((Job*) &(MemoryMapTable[address].ptr)))(OPERATION_WRITE,
					address, &value);
		} else {
...
int MemoryMap::readMemory(unsigned char address, unsigned char* value) {
	...
	if (RWOP & OPERATION_READ) {
		if (RWOP & OPERATION_JOB) {
			(*((Job*) &(MemoryMapTable[address].ptr)))(OPERATION_READ, address,
					value);
		} else {
...
MemoryMap::MemoryMap() {
	...
	for (i = 0; i < count_of_slots; i++) {
		memcpy(&(MemoryMapTable[i]), (void*) &initialCommand,
